...
describe('invader AI (spec risk #2)', function () {
	...
	// This suite drives the BARE mockup server — no DojoWorld — because what it
	// checks is engine behaviour, not the facade. Hence addRoomObjectUnchecked
	// (the raw insert DojoWorld.addObject wraps) rather than world.addObject:
	// there is no DojoWorld here to call, and these docs are written by hand on
	// purpose so the test controls exactly what the engine sees.
	before(async function () {
		...
		await server.world.addRoomObjectUnchecked('W0N0', 'controller', 10, 10, { level: 0 });

		const bot = await server.world.addBot({
			username: 'victimOwner', room: 'W0N0', x: 40, y: 40,
			modules: { main: 'module.exports.loop = function () {};' }
		});
		await server.world.addRoomObjectUnchecked('W0N0', 'creep', 25, 25, {
		...
		await server.world.addRoomObjectUnchecked('W0N0', 'creep', 27, 25, {
